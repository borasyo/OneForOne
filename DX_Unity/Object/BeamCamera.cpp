...
const int nMoveTime = 10;
const int nWaitTime = 400;
const float fMoveDistance = 15.0f;
const float fMoveHeight = 2.0f;
const float fMoveVertical = 6.0f;

const float fInitHeight = 1.0f;
const float fInitDistance = 15.0f;
const float fInitVertical = 11.0f;

bool BeamCamera::Start(){
...
void BeamCamera::Update(){
	...
	if(m_bOn) {
		...
		if(m_nTime < nWaitTime) {
			...
			if(m_fHeight <= fInitHeight + 0.01f) {
...
void BeamCamera::Sway( int nTime, float fSwayRange, bool bBeside, bool bUp ) {
