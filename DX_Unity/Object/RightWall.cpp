...
// --- マクロ
#define	FIELD_SIZE_Y	(100.0f) // 地面のサイズ(X方向)
...
// ---- 準備
bool RightWall::Start(){
	
	m_pD3DDevice = CDirect3D::Create()->GetDirect3DDevice();
	m_bCollision = false;
	
	// 頂点情報の作成
	MakeVertex(4, 4, FIELD_SIZE_Y, FIELD_SIZE_Z);
...
HRESULT RightWall::MakeVertex(int nNumBlockY, int nNumBlockZ, float fSizeBlockY, float fSizeBlockZ){
	
	// 頂点数（縮退ポリゴン用を考慮する）
	m_nNumVertex = ( nNumBlockY + 1 ) * nNumBlockZ * 2 + ( nNumBlockZ - 1 ) * 2;
	
	// * インデックス数の設定（縮退ポリゴンを考慮する）*
	m_nNumVertexIndex = ( nNumBlockY + 1 ) * nNumBlockZ * 2 + ( nNumBlockZ - 1 ) * 2;

	// ポリゴン数（縮退ポリゴン用を考慮する）
	m_nNumPolygon = nNumBlockY * nNumBlockZ * 2 + ( nNumBlockZ - 1 ) * 4;
	...
	{//頂点バッファの中身を埋める
		...
		for( int CntZ = 0; CntZ < nNumBlockZ + 1; CntZ ++ )
		{
			...
			{
				pVtx[CntZ * (nNumBlockY + 1) + CntY].vtx.x = FIELD_POS;
				pVtx[CntZ * (nNumBlockY + 1) + CntY].vtx.y = -nNumBlockY / 2.0f * fSizeBlockY + CntY * fSizeBlockY;
				pVtx[CntZ * (nNumBlockY + 1) + CntY].vtx.z = +nNumBlockZ / 2.0f * fSizeBlockZ - CntZ * fSizeBlockZ;
